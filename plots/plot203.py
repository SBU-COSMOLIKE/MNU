...
parameter = [u'omegam',u'sigma8',u'mnu2', u'H0', u'w0', u'w1',u'w2']
chaindir=os.getcwd()

analysissettings={'smooth_scale_1D':0.3,'smooth_scale_2D':0.3,'ignore_rows': u'0.5',
...
samples.saveAsText(chaindir + '/.VM_P203_TMP1')
# --------------------------------------------------------------------------------
...
samples.addDerived(p.s8omegamp5/0.5477225575,name='SS8',label='{S_8}')
samples.addDerived(p.mnu,name='mnu2',label='{\\sum m_\\nu}', range=[0.06,0.7])
samples.saveAsText(chaindir + '/.VM_P203_TMP2')
# --------------------------------------------------------------------------------
...
samples.addDerived(p.s8omegamp5/0.5477225575,name='SS8',label='{S_8}')
samples.addDerived(p.mnu,name='mnu2',label='{\\sum m_\\nu}', range=[0.06,0.7])
samples.saveAsText(chaindir + '/.VM_P203_TMP3')
# --------------------------------------------------------------------------------

#GET DIST PLOT SETUP
g=gplot.getSubplotPlotter(chain_dir=chaindir,analysis_settings=analysissettings2,width_inch=5.0)
...
param_3d = None
g.triangle_plot([chaindir + '/.VM_P203_TMP1', 
                 chaindir + '/.VM_P203_TMP2',
                 chaindir + '/.VM_P203_TMP3'],
parameter,
plot_3d_with_param=param_3d,line_args=[
{'lw': 1.0,'ls': 'solid', 'color':'lightcoral'},
{'lw': 1.7,'ls': 'solid', 'color': 'indigo'},
{'lw': 1.3,'ls': '--', 'color':'black'},
],
contour_colors=['lightcoral','indigo','black'],
contour_ls=['solid','--','solid'], 
contour_lws=[1.0,1.7,1.3],
filled=[True,False,False],
shaded=False,
legend_labels=[
    '$3$-bin w-const P1 + SPT TTTEEE + ACTDR6 $\phi\phi$',
    '$3$-bin w-const P2 + SPT TTTEEE + ACTDR6 $\phi\phi$',
    '$3$-bin w-const P3 + SPT TTTEEE + ACTDR6 $\phi\phi$'
  ],
no_extra_legend_space=True,
legend_loc=(0.47,0.85)
)
